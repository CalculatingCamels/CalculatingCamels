...
var Hyperlapse = function(container, params) {
	...
	var self = this,
		...
		_max_points = _params.max_points || 150,
		_fov = _params.fov || 70,
		_zoom = _params.zoom || 1,
		_loop = ("undefined" !==  typeof _params.loop) ? _params.loop : false,
		_lat = 0, _lon = 0,
  ...
  _renderer = isWebGL() ? new THREE.WebGLRenderer() : new THREE.CanvasRenderer();
	...
	var loop = function() {
		...
		if(_forward) {
			if(++_point_index == _h_points.length) {
				...
				if (!_loop) {
					self.pause();
					if (self.onPlayComplete) self.onPlayComplete();
				}
			}
		} else {
			if(--_point_index == -1) {
	 ...
	 * @type {Number}
	 */
	this.millis = _params.millis || 200;
